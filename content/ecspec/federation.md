...
- 送信のタイミングは大きく、受注時、売上確定時などがありますが、これはOMS（Order Management System注文管理システム）がどこにあるかで変わります。
- ECの受注管理がOMSを兼ねる場合は、出荷して売上確定後に経理システムに対して、受注情報（売上情報）を送信する形になります。
- ECの受注管理がOMSを兼ねない場合（基幹システム側がOMSの場合）は、受注後にOMS側へ受注情報（売上情報）を連携し、以後OMS側で変更される「受注ステータス情報等」をEC側へ連携します。
...
- この場合、OMS側で明細を編集した際は、EC側にもその情報を連携する必要があります。
- 注文明細の詳細（商品情報、個数、価格）までEC側に連携するかどうかは、システムの連携しやすさで判断されます。
- 個数の変更や金額の書き換えをEC側に連携しようとすると、計算式そのものを連携する必要があり、システム連携の難易度があります。
- その為、OMS側ではキャンセルという形にして、顧客に再注文を依頼するケースもあります。
...
- 一般的には、基幹システム側で管理しているのは商品名やJANコード、売価等の最小限の項目であるケースが多いです。
