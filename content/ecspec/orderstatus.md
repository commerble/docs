...
また、このステータスに応じて、入金確認や決済確定、配送会社における送り状番号の送信、ポイント付与等の「業務」が発生していきます。
...
受注後、「引当済」までにユーザーが注文をキャンセルするケースがあるので、以下の仕様を考慮します。
...
- 離島や特定住所の場合は、そもそも送付して問題ないか、送料の再計算が必要かなどを調整します。
...
- 注文をそもそもユーザーが変更できるのか、変更に合わせて決済方法や金額の修正、付与ポイントの修正、など手間が多く発生し得ます。これらの反映が自動化されていないような場合は、変更を受け付けず、キャンセル後再注文してもらうような手法をとる場合もあります。
...
自動化によって、業務効率を上げることが可能な部分でもあります。
...
在庫の調整や、顧客との調整（納期の変更、注文のキャンセルなど）をします。
...
クロネコヤマトや佐川急便では、配送状況を確認するWEBサービスを用意している。
