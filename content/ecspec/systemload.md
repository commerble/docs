...
- 前提としてアプリケーション側でパフォーマンスの悪いクエリや処理を確認して最適化し、DBにインデックスを張るなどのパフォーマンスチューニングや、要件に応じたコーディングができていること。

- システム負荷対策において、単にサーバー台数を増やすことは、チューニングできていない不完全なインフラを拡張することになり不利益が大きいため、アプリケーションが最適化されていること。

## 計測＝何がボトルネックになっているのか

- まず何がボトルネックになっているのかを計測します。
- 計測は、負荷問題が起きてからではなく、日常的に計測していく必要があります。
- 日常的な計測では、CPU負荷、メモリの使用量、1受注当たりの速度などの指標を記録し、処理速度が閾値を超えた場合はアラートを通知することが求められます。
...
- システムは事業の成長過程に応じて増強できる必要があります（最初から必要以上の構成は不要です）。
...
- 一般的にWEBサーバーは水平にスケールアウトしやすく、データベースなどはスケールアップして対応するケースが多いです。※ソーシャルゲームの場合はシャーディングという形をとって、データベースサーバーを水平に足していくやり方があります。
- 上記が一般的なシステム負荷対処ですが、ECシステム負荷対策の基本方針として、オリジンサーバー（購入時の受注などECのコア業務を行うサーバー）への負荷を減らすことが重要です。

![オリジンサーバーへの負荷を減らす](../media/systemload-base.jpg)

- オリジンサーバーへの負荷を減らすために、キャッシュの多用やサービス分割を行っていきます。
- キャッシュはシステム上の複数の層で行えます（後述）。
- サービス分割とは、検索やレコメンドのサービスを別に切り出したり、通販基幹システムをECフロントとは別にしたりすることです。
- 他にも使用するプロトコルなどでシステム負荷は変わるため、ボトルネックの計測結果に応じて対応します。

## キャッシュ

CDN（Contents Delivery Network）、WEBサーバーでのキャッシュ、DBサーバーでのキャッシュ、クライアントでのキャッシュなど、キャッシュ可能な場所は多岐にわたります。
...
- キャッシュ効率を上げる設定も検討します。URLに付加する検索クエリパラメータでURLが異なる場合は、パラメータごとにキャッシュするのか、パス単位でキャッシュするのかを検討します。
- Google botやその他botのクロールによる大量アクセスがあっても問題がなくなります。
- CDNはCloudfrare、Fastly、Cloudfront、Akamai、KeyCDN、Stackpath, Verizonなど複数社が提供しており、要件に合うものを選ぶと良いでしょう。
...
- N時スタートのセールに合わせてキャッシュをパージするなど、そういった運用部分も含めて設計します。
...
- この際の商品全件取得は、件数やデータ構造によっては負荷が高いため、注意します。
- 検索サービスとECを切り離すと、商品情報や在庫情報の連携（商品追加や価格をはじめとした商品情報の変更、在庫の変更）にタイムラグが生まれます。タイムラグを考慮した仕様や画面仕様にすることが重要です。
...
- 場合によっては、レンダリングをサーバーサイドからクライアントサイドに変更することで負荷を軽減できます。
