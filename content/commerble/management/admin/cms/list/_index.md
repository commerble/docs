...
**新規作成**ボタンを押すと、新規にデータを登録できます。

**リサイクル**ボタンを押すと、過去に削除したデータを復元する画面が表示されます。
...
※以下はバナーのリストデータを開いたときの例です。他のリストデータでは、`ColumnName`、`ViewName` の項目が異なります。
...
4. アップロードされたCSVのデータに問題がある場合は、エラーが表示され、**取り込み**ボタンを押せなくなります。**また、エラーのある行は空白行として表示されます**。
...
## CSVの列は可変。ただし主キーの列は必須
CSVファイルに用意する列は必ずしも全てのフィールドを用意する必要はありません。  
必ず必要な列は、データを一意に識別する主キーとなるフィールドです。データを新規登録する場合は、入力必須のフィールドの列も必要です。  
- **IsPrimaryKey** がチェックされている列が主キーです。
- **IsNullable** がチェックされていない列は入力必須です。

![主キー・入力必須](primary-nullable.png)

## 未登録の場合は新規登録、登録済の場合は更新
アップロードするCSVデータが、Commerbleに未登録の場合は新規登録されます。Commerbleに登録済の場合は更新されます。

## 一括アップロード（Zip圧縮）
登録するファイルをZip圧縮してアップロードします。   

バナー画像を2つ（`top-banner.png`、`side-banner.png`）アップロードする例です。
1. バナーのリスト管理画面で、CSVファイルのテンプレートをダウンロードします。テンプレートは、一括アップロード画面の **template download** リンクからダウンロードできます。
2. ダウンロードしたCSVファイル名を `2020banner.csv` に変更します。CSVファイル名は任意ですが説明の都合上 `2020banner.csv` としています。
3. CSVファイルと同じ名前の空ディレクトリ `2020banner` を作成します。ディレクトリ名はCSVファイル名と同じにします。
...
7. 以上でファイルの準備は完了です。作成したディレクトリ `2020banner` をZip圧縮します。圧縮後のファイル名は、CSVファイル名と同じ `2020banner.zip` にします。
...
3. アップロードするファイルを圧縮してアップロードする予定のディレクトリに集めます。  
  *ルートディレクトリの名前はCSVファイルと同じ名前（SiteFiles）にしてください。*

![windir](win-dir.png)

4. ルートディレクトリをZip圧縮します。  
  *Zipファイルの名前はCSVファイルと同じ名前（SiteFiles）にしてください。*
