...
受注キャンセル時は、以下のオプションを指定します。

- **出荷可能数**  
  在庫引当済み商品についての処理を選択します。
  - 加算しない
  - 通常加算する（出荷可能数へ加算）
  - 予備加算する（予備在庫へ加算）
- **販売可能数**  
...
オプション設定が済んだら**受注キャンセル**ボタンからキャンセル処理を実行します。

<!-- textlint-disable ja-technical-writing/ja-no-mixed-period -->
{{% alert color="warning" %}}
セット品の場合、「子商品」は**出荷可能数**、「親商品」は**販売可能数**へ在庫を戻します。  
...
<!-- textlint-disable ja-technical-writing/ja-no-mixed-period -->
{{% alert color="warning" %}}
合計金額が変更された場合、[決済基本情報](.#決済基本情報) の金額も変更してください。  
...
### 着荷後返品
着荷後の返品は、受注情報から返品情報データを手動で作成します。  
対象となる受注を検索し、受注情報を表示します。

まずは受注明細下部にある[返品一覧](.#返品一覧) から返品情報を登録します。
